 ...
 */
int swapBytes(int x) {
    int b = 255; //...0011111111
    int rightByte = ~((~x) | ~b);
    int secondByte = ~((~x) | ~(b << 16));
    int firstByte =  ~((~x) | ~(b << 8));
    int thirdByte =  ~((~x) | ~(b << 24));
    int result = thirdByte | (rightByte << 16) | firstByte | (secondByte >> 16);
    return result;
}
...
int boundedMult(int x, int n) {
    ...
    signBit = ~(!!signBit) + 1;
    int ret = (signBit & lowerBound) | (~signBit & upperBound);
    int tooBig = (signBit & (((~x + 1) >> 30) & 1 ) ) | (~signBit & ((x >> 30) & 1 ) );

    tooBig = ~(!!tooBig) + 1;
    printf("tooBig: %d", tooBig);



    return (tooBig & ret) | (~tooBig & (x << 1));
}
